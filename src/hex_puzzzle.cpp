...
//			if (!noMouse)
	...
	void UpdateKeys()
	{
		...
		{
			if (keyState[SDLK_z] || keyState[SDLK_BACKSPACE] || keyState[SDLK_LSHIFT] || keyState[SDLK_LALT] || keyState[SDLK_u]) 
			{
				Undo();
				return;
			}
		...
		if (isMap && !editMode)
		{
			...
			else if ((keyState[SDLK_RETURN] | keyState[SDLK_KP5] | keyState[SDLK_SPACE] | keyState[SDLK_LCTRL] | keyState[SDLK_KP_ENTER]) & 2) 
			{
	...
	bool HandleKey(int key, int mod)
	{
		...
		else if (key==SDLK_KP5 || key==SDLK_SPACE || key==SDLK_LCTRL || key==SDLK_RETURN || key==SDLK_KP_ENTER)
		{
			noMouse=1;
			if (win && winFinal)
...
#endif
		else if (key=='z' || key==SDLK_BACKSPACE || key==SDLK_LSHIFT || key==SDLK_LALT || key==SDLK_DELETE || key=='u') 
		{
			if (!isMap)
				Undo();
		}
