...
/*EMPTY*/
_("Basic controls:|Move around with the numeric  keypad. Use Y, B or R to  undo mistakes.    The 'select' key brings up a  menu from which you can restart if you get stuck."),
/*NORMAL*/
0,
/*COLLAPSABLE*/
...
/*TRAMPOLINE*/
_("You can bounce on the purple trampoline tiles to get around. But try not to fall in the water. If you do, remember you can undo with Y, B, or R!"),
/*SPINNER*/
...
_("Collecting the golden jump pickups will allow you to do a big vertical jump. Try it out on different types of tiles. Use X or A to jump."),
		...
		for (int i=0; i<MAX_GAMESLOT; i++)
		{
			...
			if (f)
			{
				...
				if (p.general.completionPercentage==100 && p.general.masteredPercentage==100)
					...
					sprintf(optionSlotName[i], _("Continue game %d (%d%%)"), i+1, p.general.completionPercentage);
